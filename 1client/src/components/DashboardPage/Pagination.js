...
class Pagination extends React.Component  {
	...
	createPaginationList () {
		...
		for (let i = 0; i < this.props.donations.length; i++) {
			let tableCols = []
			tableCols.push(<td className="Donating__tb-col">{this.props.donations[i].volunteerName}</td>)
			tableCols.push(<td className="Donating__tb-col">{this.props.donations[i].email}</td>)
			tableCols.push(<td className="Donating__tb-col">{this.props.donations[i].amount}</td>)
			tableCols.push(<td className="Donating__tb-col">{this.props.donations[i].message}</td>)
			tableCols.push(<td className="Donating__tb-col">{JSON.stringify(this.props.donations[i].date).substring(1, 11)}</td>)
	...
	createPaginationNav () {
		console.log(this.props.paginationPages)
			if (this.props.paginationPages > 1) {
				...
				for (; i <= (Number(this.props.current) + 4) && i <= this.props.paginationPages; i++) {
	...
	render() {
		...
		return (
			...
			<div className="Donating__pagination">
