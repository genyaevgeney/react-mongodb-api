...
class DonationService {
  ...
  async getUserPaginationPageData(id, login) {
    ...
    const collectionCount = await donationRepository.getUserCollectionCount(login)
    let paginationPages = Math.ceil(collectionCount / perPage);
    if (paginationPages === 0) return { emptyDashboard }
    const donations = await donationRepository.getUserPaginationPageData(perPage, page, login)
    ...
    const amountForThisMonth = await donationRepository.getUserAmountForThisMonth(startDate, endDate, login)
