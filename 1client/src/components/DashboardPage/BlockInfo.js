	...
	createInfoBlocks() {
		...
		for(let i = 0; i < 3; i++) {
			...
			if (this.props.blockInfoData[i].length === 3) {
				blockChilds.push(<p className="Donating__block-headline">{this.props.blockInfoData[i][0]}</p>)
				blockChilds.push(<p className="Donating__block-price">{this.props.blockInfoData[i][1]}</p>)
				blockChilds.push(<p className="Donating__block-person">{this.props.blockInfoData[i][2]}</p>)
			} else {
				blockChilds.push(<p className="Donating__block-headline">{this.props.blockInfoData[i][0]}</p>)
				blockChilds.push(<p className="Donating__block-price">{this.props.blockInfoData[i][1]}</p>)
			}
